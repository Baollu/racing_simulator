...
# but works fine on 3.11 when installed from source.
...
echo "      (bypassing strict Python version constraint)"
...
# Relax Python version constraint for 3.11 compatibility
sed -i 's/python_requires=">=3.10.1,<=3.10.12"/python_requires=">=3.10.1"/' "$TMPDIR/ml-agents/ml-agents-envs/setup.py"

# Also relax numpy constraint for broader compatibility
...
echo ""
echo "Verify with: python3 -c 'from mlagents_envs.environment import UnityEnvironment; print(\"mlagents_envs OK\")'"
